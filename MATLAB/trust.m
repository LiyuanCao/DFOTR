...
if key == 0
   if seceqn(laminit,eigval,alpha,delta) > 0
      [b,~,count] = rfzero('seceqn',laminit,itbnd,eigval,alpha,delta,tol);
%       vval = abs(seceqn(b,eigval,alpha,delta));
      if abs(seceqn(b,eigval,alpha,delta)) <= tol2
         ...
         w = eigval + lambda;
         coeff = alpha ./ w;
         coeff(isnan(coeff))=0; % 0/0 = NaN from previous step
         s = V*coeff; 
         nrms = norm(s);
   ...
   if (key > 2) 
      arg = abs(eigval + lambda) < 10 * eps * max(abs(eigval),ones(n,1));
      alpha(arg) = 0;
   end
   w = eigval + lambda;
   coeff = alpha ./ w;
...
%
M = bsxfun(@plus, eigval, lambda'); 
M = bsxfun(@rdivide, alpha, M);
M = M.^2;
value = 1 / sqrt(sum(M));
value(isnan(value)) = 0;
value = 1/delta - value';
