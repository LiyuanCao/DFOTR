...
class sample:
    ...
    def poisedness_improvement_points(self, p, par):
        # p is the number of points requested 

        # distance to the center (We do not use self.distance because it has the distance from the center to itself. )
        distance = np.linalg.norm(self.Ycentered, axis=1)

        # the lambdas we want to maximize
        idx = np.argsort(distance)[-p:]
        # idx = np.random.randint(self.m - 1)
        
        # inv(MLMLT + MQMQT)
        temp = np.dot(self.Ycentered, self.Ycentered.T) + np.dot(self.Ycentered, self.Ycentered.T)**2/2
        temp = np.linalg.inv(temp)

        # MQ
        MQ = map(lambda x: quadratic_terms(x), self.Ycentered)  

        # Lagrange basis
        BL = np.dot(temp[idx], self.Ycentered)
        BQ = np.dot(temp[idx], MQ)

        # the poisedness improvement points
        points = np.empty((p, self.n))
        # finding these points involves solving trust region subproblems
        for i in range(p):
            model = quadraticModel(self.n)
            model.g = - BL[i]
            model.H = np.zeros((self.n, self.n))
            model.H[np.tril_indices(self.n)] = BQ[i]
            model.H = - (model.H + model.H.T - np.diag(np.diag(model.H)))

            points[i],val = trust_sub(np.zeros(self.n), model, par)
            print(val)
